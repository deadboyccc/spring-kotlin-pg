...
class UserReactiveController(private val repo: UserReactiveRepository) {
    @PostMapping
        fun create(@RequestBody req: UserRequestR) =
    ...
    @PutMapping("/{id}")
        fun update(@PathVariable id: String, @RequestBody req: UserRequestR) =
            repo.findById(id)
    ...
    @DeleteMapping("/{id}")
        fun delete(@PathVariable id: String) =
            repo.existsById(id)
                .flatMap { exists ->
                    if (exists) repo.deleteById(id).then(Mono.just(ResponseEntity.noContent().build()))
